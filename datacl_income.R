...
y1.raw <- log(data$mwage_atax) #data$lnwage
y1.raw[data$mwage_atax==0] == 0
...
x.d.raw <- cbind( data$urban, data$familysize, data$spouse, data$military1, data$party1, data$evsmoke1, data$health, data$minority, data$agri_job,
                   data$stateown, data$collective,data$private, data$foreign, data$formal,
                  data$part_time, data$bcollar, 
                  data$wcollar, data$service,  data$BJ, data$TJ, data$HeB, data$ShX, data$NMG, data$LN, data$JN, data$HLJ, data$SH, data$JS, data$ZJ, data$AnH, data$FJ, 
...
#To be continued
# delete NA rows
data.raw = cbind(y1.raw, y2.raw, y3.raw, y4.raw, D1.raw, D2.raw, x.c.raw, x.d.raw, z1.raw, z2.raw)
...
w_ini.label <- c('urban', 'familysize', 'spouse', 'military1', 'party1', 'evsmoke1', 
                 'health', 'minority', 'agri_job', 'stateown', 'collective', 'private',
                 'foreign', 'formal', 'part_time', 'bcollar', 'wcollar', 'service',  'BJ',
...
save(y1, y2, y3, y4, y5, D1, D2, D3, D4, D5, x.d, x.c, x.zl, z1, z2, w.d.cross, w.cross, file = "OBhimdimcfps2018v2.RData")
save(OBhim.label, file = "OBhimdimcfps2018LBv2.RData")
save.image(file="OBhimdimcfps2018v2_all.RData")