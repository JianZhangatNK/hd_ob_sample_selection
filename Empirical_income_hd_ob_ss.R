...
#  Features: Outcome variable Y is continuous;
# 
#
# variables:
# y1 - hourly wage, y2 - monthly wage, y3 - yearly wage, y4 - health
...
y = y1[ste.sample.index]
y.label = "lnhourly wage"
...
###### IV settings
z.data = cbind(z1,z2,z1^2,z2^2) # cbind(z1,z1^2)
...
PI0hat_ii = matrix(NA, n, n)
PI1hat_ii = matrix(NA, n, n)
for (ii in c(1:n)) {
  cat('ii =', ii, '\n')
...
save.image(file = "OBhimdimcfps2018Mainv2_temp1.RData")
#load("OBhimdimcfps2018Mainv2_temp1.RData")
...
save.image(file = "OBhimdimcfps2018Mainv2_temp2.RData")
...
save.image(file = "OBhimdimcfps2018Mainv2_temp3.RData")
...
varlist0 = c("n.total.sample", "ste.sample.index", "y", "y.label", "D", "D.label",
            "w", "w.d", "z", "x", "x.c.ep",  "ylim.list",  "b.xi",
            "n", "datafolds.list", "f", 'S', 's', 'rho_0_min', 'rho_1_min',
            'F.star.hat_y0', 'F.star.hat_y1', 'alpha.aml', 'alpha1.S.aml',
            'alpha0.S.aml')
clusterExport(cl, varlist0, envir = .GlobalEnv)

##### Main Estimation #####

Emp_Est <- function(ncount){
  ...
  for(k in 1:f){
    ...
    # conditional CDF
    x0poly = xpoly.nfold[(D.nfold==0)&(s.nfold==1),]
    x1poly = xpoly.nfold[D.nfold==1,]
    y0.nfold = y.nfold[(D.nfold==0)&(s.nfold==1)]
  ...
  Fyx.c = Fyx[(D==1)&(s==1)]
  Fyx0 = Fyx[(D==0)&(s==1)]
  Fhatyx.c.aml = mean(Fyx.c) + mean(alpha.aml[(D==0)&(s==1)] * (yvar0-Fyx0))
  
  # MBootstrap:
  Fhatyx.c.aml.B = (b.xi[,(D==1)&(s==1)]%*%Fyx.c)/sum((D==1)&(s==1)) + (b.xi[,(D==0)&(s==1)]%*%(alpha.aml[(D==0)&(s==1)] * 
                                                     (yvar0-Fyx0)))/(sum((D==0)&(s==1)))
  ...
  ## Output
  return(list(F1yhat, Fhatyx.c.aml, StEffectyx.c.aml,
              F1yhat_star, Fhatyx.c.aml_star, StEffectyx.c.aml_star,
              F1yhat.B, Fhatyx.c.aml.B, StEffectyx.c.aml.B,
              F1yhat_star.B, Fhatyx.c.aml_star.B, StEffectyx.c.aml_star.B))
}

## Simulation Starting...
ncount.list = c(1:ngrid)
Emp_Est_Output = parLapply(cl, ncount.list, Emp_Est)

## Store Output Results
F1yhat = rep(NA, ngrid)
Fhatyx.c.aml = rep(NA, ngrid)
StEffectyx.c.aml = rep(NA, ngrid)

F1yhat_star = rep(NA, ngrid)
Fhatyx.c.aml_star = rep(NA, ngrid)
StEffectyx.c.aml_star = rep(NA, ngrid)

F1yhat.B.mat = matrix(NA, B, ngrid)
Fhatyx.c.aml.B.mat = matrix(NA, B, ngrid)
StEffectyx.c.aml.B.mat = matrix(NA, B, ngrid)

F1yhat_star.B.mat = matrix(NA, B, ngrid)
Fhatyx.c.aml_star.B.mat = matrix(NA, B, ngrid)
StEffectyx.c.aml_star.B.mat = matrix(NA, B, ngrid)

for (ncount in ncount.list) {
  output.list = Emp_Est_Output[[ncount]]
  F1yhat[ncount] = output.list[[1]]
  Fhatyx.c.aml[ncount] = output.list[[2]]
  StEffectyx.c.aml[ncount] = output.list[[3]]
  F1yhat_star[ncount] = output.list[[4]]
  Fhatyx.c.aml_star[ncount] = output.list[[5]]
  StEffectyx.c.aml_star[ncount] = output.list[[6]]
  F1yhat.B.mat[,ncount] = output.list[[7]]
  Fhatyx.c.aml.B.mat[,ncount] = output.list[[8]]
  StEffectyx.c.aml.B.mat[,ncount] = output.list[[9]]
  F1yhat_star.B.mat[,ncount] = output.list[[10]]
  Fhatyx.c.aml_star.B.mat[,ncount] = output.list[[11]]
  StEffectyx.c.aml_star.B.mat[,ncount] = output.list[[12]]
}


## B-standard deviation
#load("OBhimdimOutput2018v3.RData")
sd.ste.aml = rep(NA, ngrid)
sd.ste.aml_star = rep(NA, ngrid)
for (ii in 1:ngrid) {
  sd.ste.aml[ii] = sd(as.vector(StEffectyx.c.aml.B.mat[,ii])) #*sqrt(B/n)
  sd.ste.aml_star[ii] = sd(as.vector(StEffectyx.c.aml_star.B.mat[,ii])) #*sqrt(B/n)
}

save.image(file = "OBhimdimcfps2018Mainv2_full_iv.RData")
...
par(mfrow=c(1,2))
## without selection
StEffectyxz.c.dml.up = StEffectyx.c.aml + 1.96*sd.ste.aml
StEffectyxz.c.dml.lo = StEffectyx.c.aml - 1.96*sd.ste.aml
plot(ylim.list, StEffectyxz.c.dml.up, type = "l", lty = 2, pch = 2, 
     col = "blue", xlab = "对数月收入", ylab = "结构效应", 
     ylim = c(min(c(min(StEffectyxz.c.dml.lo),0)), max(c(max(StEffectyxz.c.dml.up),0))), 
     main = "未考虑样本选择")
lines(ylim.list, StEffectyx.c.aml, type = 'l', pch = 2, col = "black")
lines(ylim.list, StEffectyxz.c.dml.lo, type = 'l', lty = 2,  pch = 2, col = "blue")
abline(h=0, lty = 'dashed', col = "red")

## with selection
StEffectyxz.c.ps.db.up = StEffectyx.c.aml_star + 1.96*sd.ste.aml_star
StEffectyxz.c.ps.db.lo = StEffectyx.c.aml_star - 1.96*sd.ste.aml_star
plot(ylim.list, StEffectyxz.c.ps.db.up, type = "l", lty = 2, pch = 2, 
     col = "blue", xlab = "对数月收入", ylab = "结构效应", 
     ylim = c(min(c(min(StEffectyxz.c.ps.db.lo),0)), max(c(max(StEffectyxz.c.ps.db.up),0))),  
     main = "基于样本选择")
lines(ylim.list, StEffectyx.c.aml_star, type = 'l', pch = 2, col = "black")
lines(ylim.list, StEffectyxz.c.ps.db.lo, type = 'l', lty = 2,  pch = 2, col = "blue")
abline(h=0, lty = 'dashed', col = "red")

## selection overestimate the counterfactual income
par(mfrow=c(1,1))
## F1(y) versus F^C(y)
plot(ylim.list, Fhatyx.c.aml_star, type = "l", pch = 2, 
     col = "blue", xlab = "y", xlim = c(min(ylim.list),max(ylim.list)), ylim = c(0, 1), main = "未考虑样本选择 v.s. 修正样本选择")
lines(ylim.list, Fhatyx.c.aml, type = 'l', pch = 2, col = "red")

legend(0.45*max(ylim.list)+0.15*min(ylim.list), 0.98, 
       legend=c("未修正样本选择偏误", "修正样本选择偏误"),
       col=c("red", "blue"), 
